...
def home():
  ...
  <div>
    ...
    <div class="box">
      ...
      <div class="box">
  <h2 style="margin:0 0 10px 0; font-size:16px;">2-1) 조건(텍스트)로 검색</h2>
  <form action="/search/text" method="get">
    <label>공구명</label>
    <input name="name" placeholder="예) 절연드라이버 / 임팩 / 니퍼" />

    <label>용도</label>
    <input name="purpose" placeholder="예) 단자 체결 / 배관 작업" />
    ...
    <div class="row">
      ...
      <div>
        <label>정렬</label>
        <select name="sort">
          <option value="recent">최근등록</option>
    ...
    <div class="hr"></div>
    <button class="btn" type="submit">조건 검색</button>
  </form>
  <div class="small">사진 검색이 “눈”이라면, 조건 검색은 “지도”입니다. 둘이 합쳐지면 분실이 줄어듭니다.</div>
</div>
...
@app.get("/search/text", response_class=HTMLResponse)
def search_text(
    name: str = "",
    purpose: str = "",
    location: str = "",
    ...
    order_map = {
        "recent": "id DESC",
        "name": "name ASC",
        "location": "location ASC, name ASC",
        "qty_desc": "qty DESC, name ASC",
    }
    order_sql = order_map.get(sort, "id DESC")
    ...
    for r in rows:
        ...
        <div class="box">
          <div style="display:flex; justify-content:space-between; gap:10px; align-items:flex-start;">
            ...
            <div class="noprint" style="min-width:130px;">
              <a href="/tools/list?{urlencode({'location': r['location']})}" style="text-decoration:none;">
                <button class="btn2" type="button">같은 위치</button>
              </a>
              <div style="height:8px;"></div>
              <form action="/events" method="post">
                <input type="hidden" name="tool_id" value="{r['id']}"/>
    ...
    # 현재 검색조건 요약
    cond = f"공구명={esc(name) or '전체'} · 용도={esc(purpose) or '전체'} · 위치={esc(location) or '전체'} · 상태={esc(status) or '전체'} · 최소수량={esc(min_qty) or '없음'}"
    ...
    <div class="box noprint">
      ...
      <div class="small">조건: {cond}</div>
    </div>

    <div class="box">
      <div style="font-weight:900; font-size:16px;">조건 검색 결과 ({len(rows)}건)</div>
      <div class="muted">최대 200건까지 표시합니다.</div>
    </div>
