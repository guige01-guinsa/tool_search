...
def tools_list(location: str = "", status: str = "", q: str = "",
    ...
    <div class="box">
      ...
      <div class="muted">필터: {esc(location) or "전체"} · {esc(status) or "전체"} · 분류 {esc(cat_l) or "전체"}/{esc(cat_m) or "전체"}/{esc(cat_s) or "전체"} · {esc(q) or "없음"}{("미분류만" if str(unclassified)=="1" else "전체")}</div>
    </div>
...
def print_a4_category_all():
    ...
    def cat_key(r):
        ...
        return " / ".join([x for x in [l, m, s] if x])
