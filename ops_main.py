...
ENTITY_ATTACHMENT_LIMITS = {"facility": 6, "inventory": 6, "office_record": 6}
ENTITY_ATTACHMENT_LIMIT_META = {
    "facility": ("첨부 이미지는", "장"),
    "inventory": ("첨부 이미지는", "장"),
    "office_record": ("첨부 파일과 이미지는", "개"),
}
...
def _upload_file(file: UploadFile) -> str | None:
    ...
    if ext not in {".jpg", ".jpeg", ".png", ".webp", ".gif", ".bmp", ".pdf", ".hwp", ".hwpx", ".doc", ".docx", ".xls", ".xlsx"}:
        ext = ".bin"
...
def _selected_uploads(files: Iterable[UploadFile]) -> list[UploadFile]:
    return [file for file in files if (getattr(file, "filename", "") or "").strip()]


def _attachment_count(conn, entity_type: str, entity_id: int) -> int:
    row = conn.execute(
...
def _attachment_limit_error(conn, entity_type: str, entity_id: int | None, files: list[UploadFile]) -> str | None:
    limit = ENTITY_ATTACHMENT_LIMITS.get(entity_type)
    if not limit or not files:
        return None
    noun, unit = ENTITY_ATTACHMENT_LIMIT_META.get(entity_type, ("첨부 파일은", "개"))
    existing_count = _attachment_count(conn, entity_type, entity_id) if entity_id else 0
    if existing_count + len(files) <= limit:
        return None
    if existing_count:
        remaining = max(limit - existing_count, 0)
        return f"{noun} 최대 {limit}{unit}까지 저장됩니다. 현재 {existing_count}{unit}이 있어 {remaining}{unit}만 추가할 수 있습니다."
    return f"{noun} 최대 {limit}{unit}까지 저장됩니다."
...
def _save_attachments(conn, entity_type: str, entity_id: int, files: Iterable[UploadFile], user_id: int | None) -> None:
    for file in _selected_uploads(files):
...
def facilities_page(request: Request):
    ...
    form_html = ""
    if can_edit:
        edit_attachment_count = len(attachments.get(edit_row["id"], [])) if edit_row else 0
        form_html = (
            ...
            + "<div><label>첨부 이미지 (최대 6장)</label><input name='files' type='file' accept='image/*' multiple>"
            + f"<p class='muted'>기존 첨부 포함 최대 6장까지 누적 저장됩니다. 현재 {edit_attachment_count}장 저장됨.</p></div>"
            + "<div class='row-actions'>"
...
def facilities_save(
    ...
    files = _selected_uploads(files)
    conn = get_conn()
    limit_error = _attachment_limit_error(conn, "facility", facility_id_i or None, files)
    if limit_error:
        conn.close()
        target = f"/facilities?edit={facility_id_i}" if facility_id_i else "/facilities"
        return _with_flash(target, limit_error, "error")
    if facility_id_i:
        conn.execute(
            """
            UPDATE facilities
...
def office_records_page(request: Request):
    ...
    if can_create or edit_row:
        ...
        if edit_row and not (can_manage_edit_row or can_update_edit_row):
            ...
            form_html += "<div style='margin-top:14px'><label>현재 첨부</label>" + attachment_gallery(current_attachments) + "</div></section>"
        else:
            form_html = (
                ...
                + "<div><label>첨부 파일 / 이미지 (최대 6개)</label><input name='files' type='file' accept='.pdf,.hwp,.hwpx,.doc,.docx,.xls,.xlsx,image/*' multiple>"
                + f"<p class='muted'>PDF, HWP, DOC, XLS, 이미지 파일을 함께 첨부할 수 있습니다. 기존 첨부 포함 최대 6개까지 누적 저장됩니다. 현재 {edit_attachment_count}개 저장됨.</p></div>"
                + "<div class='row-actions'>"
                + "<button class='btn primary' type='submit'>저장</button>"
            ...
            if edit_row:
                ...
                if current_attachments:
                    form_html += (
                        f"<form action='/office-records/attachments/delete/{edit_row['id']}' method='post' class='stack' style='margin-top:10px;'>"
                        + "<p class='muted'>삭제할 첨부를 선택하면 한 번에 정리할 수 있습니다.</p>"
                        + attachment_selector(current_attachments)
                        + "<div class='row-actions'>"
                        + "<button class='btn warn' type='submit' onclick=\"return confirm('선택한 첨부를 삭제하시겠습니까?')\">선택 첨부 삭제</button>"
        ...
        if edit_row and can_update_edit_row:
            ...
            form_html += (
                ...
                + "<div><label>추가 첨부 (최대 6개)</label><input name='files' type='file' accept='.pdf,.hwp,.hwpx,.doc,.docx,.xls,.xlsx,image/*' multiple>"
                + f"<p class='muted'>기존 첨부 포함 최대 6개까지 유지됩니다. 현재 {edit_attachment_count}개 저장됨.</p></div>"
...
@app.post("/office-records/attachments/delete/{record_id}")
def office_records_delete_attachments(
    request: Request,
    record_id: int,
    attachment_ids: List[str] = Form(default=[]),
):
    ...
    attachment_ids_i = _parse_id_list(attachment_ids)
    if not attachment_ids_i:
        return _with_flash(f"/office-records?edit={record_id}", "삭제할 첨부를 선택해 주세요.", "error")
    ...
    if not _can_manage_office_record(user, record):
        conn.close()
        return _with_flash(f"/office-records?edit={record_id}", "이 행정업무의 첨부를 삭제할 권한이 없습니다.", "error")
    deleted_count = _delete_selected_attachments(conn, "office_record", record_id, attachment_ids_i)
    conn.commit()
    conn.close()
    if not deleted_count:
        return _with_flash(f"/office-records?edit={record_id}", "삭제할 첨부를 찾을 수 없습니다.", "error")
    return _with_flash(f"/office-records?edit={record_id}", f"선택한 첨부 {deleted_count}건이 삭제되었습니다.", "ok")
...
def office_records_save(
    ...
    conn = get_conn()
    limit_error = _attachment_limit_error(conn, "office_record", record_id_i or None, files)
    if limit_error:
        conn.close()
        target = f"/office-records?edit={record_id_i}" if record_id_i else "/office-records"
        return _with_flash(target, limit_error, "error")
...
def office_records_update(
    ...
    limit_error = _attachment_limit_error(conn, "office_record", record_id, files)
    if limit_error:
        conn.close()
        return _with_flash(f"/office-records?edit={record_id}", limit_error, "error")
...
def inventory_save(
    ...
    files = _selected_uploads(files)
    conn = get_conn()
    limit_error = _attachment_limit_error(conn, "inventory", item_id_i or None, files)
    if limit_error:
        conn.close()
        target = f"/inventory?edit={item_id_i}" if item_id_i else "/inventory"
        return _with_flash(target, limit_error, "error")
